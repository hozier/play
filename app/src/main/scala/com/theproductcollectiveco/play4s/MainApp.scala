...
import com.theproductcollectiveco.play4s.config.{AppConfig, ApiKeyStoreConfig, AuthConfig, storeCredentials}
...
object MainApp extends ResourceApp.Forever {
  ...
  override def run(args: List[String]): Resource[IO, Unit] =
    for {
      ...
      AppConfig(ApiKeyStoreConfig(_, AuthConfig(apiKey, credentialsFilePath)), _) = summon[AppConfig]
      _                                                                          <- apiKey.storeCredentials(credentialsFilePath.mkString)
