...
object AuthService {
  ...
  def make[F[_]: Async: Logger](using jwtProvider: JwtProvider[F], otpProvider: OtpProvider[F]): ServiceAuthApi[F] =
    new ServiceAuthApi[F] {
      ...
      override def initiateOtpRequest(requester: GenericHandle): F[Unit] =
        for {
          otp <- otpProvider.initializeOtp(requester)
          _   <- Logger[F].info(s"Initialized otp: $otp") // todo: send the otp via sms or email client integration
        } yield ()
